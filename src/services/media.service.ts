...
async function uploadImageCateToS3(categoryId: string, file: Express.Multer.File): Promise<void> {
  // try {
...
async function getFileCateFromS3(categoryId: string): Promise<string> {
  // try {
  //   const category = await categoryService.getCategoryById(categoryId)
  //   if (!category) {
  //     throw new Error('Not found')
